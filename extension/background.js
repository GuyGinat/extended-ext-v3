                ...
                chrome.identity.launchWebAuthFlow(
                    ...
                    function (redirect_url) {
                        if (chrome.runtime.lastError) {
                            console.log(
                                `error signing in: ${chrome.runtime.lastError}`
                            );
                        } else {
                            let id_token = redirect_url.substring(
                                redirect_url.indexOf("id_token=") + 9
                            );
                            id_token = id_token.substring(
                                0,
                                id_token.indexOf("&")
                            );
                            ...
                            if (
                                ...
                                fetch(
                                    "http://localhost:3001/users/extension-sign-in",
                                    {
                                        method: "post",
                                        headers: {
...
/**
 * Fired each time a tab is opened or reloaded
 * 
 * @listens onUpdated
 */
...
/**  
 * opens omnibox for quick search,
 * after entering your search phrase press enter.
 * To use press 'E' -> 'Tab' 
 * 
 * @listens omnibox
 */
...
/**
 * Parse YouTube video id from url, gets captions,
 * vists, and sends the to server
 * 
 * @param {chrome.tab} tab the tab object of the window
 */
...
/**
 * Gets all browsing history of user and saves the websites
 * without any text
 */
function getHistory() {
    let pages = [];
    chrome.history.search(
        { text: "", maxResults: 0, startTime: 0 },
        function (data) {
...
/**
 * Gets all visits of a user in a page
 * 
 * @param {url} url the url of the page.
 * @returns {[object]} an array of objects with timestamps and referals.
 */
function getPageHistory(url) {
    ...
    chrome.history.getVisits({ url: url }, (results) => {
        for (let i = 0; i < results.length; i++) {
            visits.push({
                date: results[i]["visitTime"],
                transition: results[i]["transition"],
            });
        }
    });
    return visits;
...
/**
 * Gets the text of a page along with all visits
 * 
 * @param {chrome.tab} tab 
 * @param {chrome.tabId} tabId 
 * @param {chrome.windowId} windowId 
 */
...
function sendPage(payload, suffix) {
    chrome.identity.getProfileUserInfo(
        ...
        function (user_info) {
            ...
            if (payload.link) {
                console.log("visited history page");
                payload["users"]["visits"] = getPageHistory(payload.link);
            }
    ...
    return new Promise((resolve) => {
        ...
        getBlacklist().then((blacklist) => {
            for (let site of blacklist) {
                regex = "^((?:https?:)?//)?((?:www|m).)?(regex)/.*".replace(
                    re,
                    site
                );
                if (url.match(regex)) {
...
function getSubtitles(videoID) {
    return new Promise((resolve, reject) => {
        let lang = "en";
        fetch(
            `https://youtube.com/get_video_info?video_id=${videoID.videoID}`,
            {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
